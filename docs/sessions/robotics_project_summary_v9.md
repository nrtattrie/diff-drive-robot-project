...
- Arduino Due V02G - **PRIMARY MCU** for motor control, encoders, low-level I/O (84 MHz ARM, 3.3V native, hardware quadrature encoder support)
- Arduino Mega 2560 (from Elegoo kit) - backup/prototyping board
...
- [x] Complete first 4 ROS 2 Jazzy tutorials on the Pi
- [x] Create first custom ROS 2 package (`diffbot_description`)
- [x] Begin URDF tutorial — basic `base_link` box model created and visualized
- [x] Set up remote visualization (Foxglove Studio on Mac via foxglove_bridge on Pi)

### In Progress:
- [ ] URDF development — expand from placeholder box to full chassis geometry (Stage 1 of roadmap)

### Upcoming:
- [ ] Add wheels, caster, and sensor frames to URDF
- [ ] tf2 coordinate frames (base_link → odom → map)
...
- **Remote visualization (primary):** Foxglove Studio (Mac app) + `foxglove_bridge` on Pi via WebSocket port 8765
  - `ros-jazzy-foxglove-bridge` installed; launch with `ros2 launch foxglove_bridge foxglove_bridge_launch.xml`
  - Connect from Mac: Foxglove Studio → Open Connection → `ws://<pi-ip>:8765`
  - Pi IP can change between boots — verify with `hostname -I`
- **RViz over SSH:** Not viable — GLX/OpenGL context fails without a physical display
- **RViz on monitor:** Still works when Pi is plugged into a screen directly
...
| HC-SR04 ultrasonic | 5V power | Trigger accepts 3.3V; **Echo outputs 5V** (see note) |
| RPi Camera Module 3 | 3.3V | Powered by RPi CSI connector |

**Key design insight:** By powering encoders from Due's 3.3V output pin, encoder signals are natively 3.3V — zero level shifting needed throughout the system. The G2 18v17 logic inputs accept 1.8–5.5V, so 3.3V works directly.

**HC-SR04 caveat:** Echo pin outputs 5V regardless of trigger voltage. Options: (1) read from RPi GPIO directly, or (2) resistor voltage divider (2.2k + 3.3k) on Echo pin to bring it to ~3.0V for Due. Decision deferred — see Open Questions.
...
9. **Motor Driver:** Pololu G2 High-Power 18v17 (4.5A continuous per channel, 1.8–5.5V logic)
...
17. **Motor Controller MCU:** Arduino Due (replacing Mega 2560 as primary)
    - 84 MHz ARM Cortex-M3, 3.3V native, hardware quadrature encoder via TC0 timer
    - Sufficient for Phase 1 diff-drive and Phase 2 self-balancing (500+ Hz PID achievable)
    - Cost: $0 (already owned)
    - Risks: discontinued by Arduino, fragile 3.3V I/O (5V = instant death), smaller community
    - Fallback: ESP32 DevKitC (~$15) — same 3.3V logic, same serial protocol, minimal firmware changes
    - Mega 2560 retained as backup/prototyping board
18. **Encoder Power Strategy:** Power encoders from Due's 3.3V output pin
    - Pololu 37D encoders accept 3.5–20V; output voltage = supply voltage
    - Powered at 3.3V → 3.3V outputs → zero level shifting needed
    - G2 18v17 logic accepts 1.8–5.5V → 3.3V safe for PWM/direction pins
...
- HC-SR04 reading strategy: RPi GPIO directly vs resistor voltage divider to Due
...
### February 26, 2026 - First URDF & Foxglove Visualization Operational
- **ROS 2 Tutorials Completed:**
  - Finished official ROS 2 Jazzy CLI tutorials and first 4 tutorial series
  - Post-tutorial roadmap established (4 stages: software-only → chassis → real hardware → autonomous nav)
- **First Custom Package — `diffbot_description`:**
  - Created `diffbot_description` ROS 2 package with proper structure (`urdf/`, `launch/`, `rviz/`)
  - URDF: `diffbot.urdf.xacro` — single `base_link` with 0.15m box geometry (placeholder for chassis)
  - Launch file: `display.launch.py` with configurable `use_rviz` and `use_gui` args
  - Package builds and runs successfully with `colcon build`
- **Remote Visualization Setup — Foxglove Studio:**
  - Investigated and abandoned X11 forwarding (XQuartz/GLX incompatibility — dead end)
  - Investigated and abandoned VNC (TigerVNC installed but replaced by cleaner solution)
  - **Decision: Foxglove Studio** as remote visualization client on Mac
  - Installed `ros-jazzy-foxglove-bridge` on Pi (v3.2.3)
  - Verified end-to-end: Pi bridge → WebSocket → Foxglove Studio on Mac → 3D panel with URDF
  - Standard startup procedure documented in memory for future sessions

### February 16, 2026 - MCU Decision & Power Architecture Finalized
- **MCU Evaluation:**
  - Compared ESP32 DevKitC, Arduino Due, Teensy 4.0, STM32, RPi Pico for Phase 1+2 needs
  - Key finding: every modern ARM MCU is 3.3V — no fast + 5V + Arduino-compatible option exists
  - Mega 2560 (16 MHz, 8-bit) too slow for Phase 2 balancing (500+ Hz PID target)
  - ESP32 has best specs but WiFi jitter concern for tight PID loops, awkward breadboard fit
  - **Decision: Arduino Due** — 84 MHz ARM, hardware quadrature encoder, $0 (already owned)
  - Fallback: ESP32 DevKitC (~$15), compatible swap if Due fails
- **Power Architecture Clarified:**
  - 3.3V logic throughout: Due native 3.3V, encoders powered from Due 3.3V pin, G2 driver accepts 1.8–5.5V logic
  - Zero level shifting needed in entire system (except HC-SR04 Echo pin)
  - Adafruit STEMMA QT boards (BNO055, INA219, APDS9960) voltage-agnostic via onboard regulators
  - Single DROK buck → 5.1V feeds RPi + Due VIN; Due regulates to 3.3V internally
- **Post-tutorial roadmap finalized** (4-stage progression from software-only to autonomous nav)
